...
def load_products(row, conn):
    ...
    for i, product in enumerate(basket):
        ...
        conn.cursor().execute(sql_query, p_values)
        ...
        if result == None:
            ...
            conn.cursor().execute(sql_query, tuple(vals))
            conn.commit()
        else:
...
def load_location(row, conn):
    ...
    l_values = str(name)
    try:
        conn.cursor().execute(sql_query, (l_values,))
    except Exception:
        conn.rollback()
    ...
    if result == None:
        ...
        conn.cursor().execute(sql_query, value)
        conn.commit()
        return loc_id
    else:
        return result[0]
...
def load_transaction(row, loc_id, conn):
    ...
    conn.cursor().execute(sql_query, values)
    conn.commit()
    return trans_id
...
def load_basket(row, trans_id, conn):
    ...
    for p_id in prod_ids:
        ...
        conn.cursor().execute(sql_query, b_values)
        conn.commit()
